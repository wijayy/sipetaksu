<x-auth-layout :title="'Edit ' . $umkm->nama">
    <form action="{{ route('umkm.update', ['umkm' => $umkm->slug]) }}" class="w-full mt-6" method="post"
        ...
        <div class="flex flex-wrap justify-start gap-4 my-6 align-middle lg:flex-no-wrap" x-data="">
            <div class="relative flex text-center align-middle rounded-md shadow-md size-24 md:size-32 lg:size-48 aspect-square"
                x-data="{
                ...
                <img :src="image" :alt="text" class="z-10 object-cover rounded-md size-full"
                    x-show="image">
                <input type="file" id="{{ __('image') }}" name="{{ __('image') }}" @change="image=imagePreview()"
                    class="sr-only">
                <label for="{{ __('image') }}" :class="{ 'opacity-100': !image, 'opacity-0': image }"
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <x-text-input id="latitude" class="block w-full mt-1" type="number" name="latitude"
                        :value="old('latitude', $umkm->latitude)" required step="0.00000000001" autocomplete="latitude" />
                    <x-input-error :messages="$errors->get('latitude')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="longitude" :value="__('Longitude')" />
                    <x-text-input id="longitude" class="block w-full mt-1" type="number" name="longitude"
                        :value="old('longitude', $umkm->longitude)" required step="0.00000000001" autocomplete="longitude" />
        ...
        <div class="flex flex-wrap justify-start gap-4 my-6 align-middle lg:flex-no-wrap" x-data="">
            @for ($i = 0; $i < 6; $i++)
                <div class="relative flex text-center align-middle rounded-md shadow-md size-24 md:size-32 lg:size-48 "
                    ...
                    <img :src="image" :alt="'Gambar' + i" class="z-10 object-cover rounded-md size-full"
                        x-show="image">
